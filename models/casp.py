...
class Casp(ContinualModel):
    ...
    def end_epoch(self, dataset, train_loader):
        ...
        if self.epoch == self.args.n_epochs:
            ...
            std_of_means_by_class = {class_id: torch.mean(torch.tensor([mean_by_class[class_id][epoch] for epoch in range(8)])) for class_id, __ in enumerate(self.unique_classes)}
